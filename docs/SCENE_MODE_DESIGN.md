...
// short: スキャンカードの補助テキスト（骨格図アイコンが主）
...
### 5.5 Phase 1: SCAN — 関係の骨組み選択

**ヘッダープロンプト**:
```
┌──────────────────────────────────────┐
│        WHAT'S THE STRUCTURE?         │  ← ピクセルフォント, ネオンイエロー
│        この文の関係の骨組みは？         │  ← DotGothic16, 小さめサブテキスト
└──────────────────────────────────────┘
```

**スキャンカードデザイン — 骨格図を主、意味を従**:

カード型ボタン。左に**骨格図アイコン（主役）**、右に**意味ラベル（補助）**。
骨格図が文型と直結するため、ラベル暗記ではなく関係の見取り図を選ぶ体験になる。
...
**骨格図アイコンの設計意図**:
| SceneType | アイコン | 視覚メタファー | 文型との対応 |
|-----------|---------|--------------|-------------|
| action | `S─V` | 主語と動詞だけの接続 | SV (第1文型) |
| state | `S=C` | 主語＝補語の等価関係 | SVC (第2文型) |
| affect | `S→O` | 主語から対象への作用 | SVO (第3文型) |
| transfer | `S→O+O` | 2つの対象（人+物）へ向かう | SVOO (第4文型) |
| transform | `S→O=C` | 対象にさらに説明がつく | SVOC (第5文型) |

特に `S→O`（SVO）と `S→O=C`（SVOC）の差が構造的に見える点が重要。
「対象がある」だけでなく、その対象に説明や状態がさらに乗るかどうかが一目でわかる。

**各ラベルの完全版（ホバー/長押し時に表示）**:
| 補助ラベル | 完全版 |
|-----------|--------|
| 主語が動く・起こる | 主語が動く・起こるだけ。対象は必須でない |
| 主語の説明になる | 主語がどんな状態か、何者かを説明している |
| 対象にはたらく | 主語の動作が対象にはたらいている |
| 相手と物の2つへ | 相手と物の2つに向かっている（渡す・教える・見せる） |
| 対象に説明がつく | 対象にさらに説明（状態・名前・認識）がつく |
